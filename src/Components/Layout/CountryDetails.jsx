    ...
    return <section className="mx-auto max-w-7xl px-0 py-0 sm:px-6 lg:px-8 sm:py-12">
        {
            country && (
                <motion.article
                    ...
                    className="relative overflow-hidden sm:rounded-xl p-0 sm:p-[2px] min-h-[100dvh] sm:min-h-0 bg-slate-900/90 sm:bg-transparent"
                >
                    <div className="relative h-full w-full sm:rounded-xl border-0 sm:border border-white/10 p-4 sm:bg-slate-900/90 sm:p-6 backdrop-blur min-h-[100dvh] sm:min-h-0">
                        ...
                        <div className="grid gap-8 lg:grid-cols-[2fr_3fr] lg:items-start pt-12 sm:pt-0">
                            <div className="space-y-6 z-10 lg:sticky lg:top-24">
                                <img src={country.flags?.png} alt={country.flags?.alt || country.name?.official} className="w-full rounded-xl object-cover shadow-lg border border-slate-700/50" />
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    {country.maps?.googleMaps && (
                                        <a
                                            ...
                                            target="_blank"
                                            rel="noreferrer"
                                            className="flex items-center justify-center w-full gap-2 rounded-xl bg-slate-800 py-3 text-xs font-semibold text-cyan-300 transition hover:bg-slate-700 hover:text-white border border-slate-700/50"
                                        >
                                            Google Maps
                                        </a>
                                    )}
                                    {country.maps?.openStreetMaps && (
                                        <a
                                            ...
                                            target="_blank"
                                            rel="noreferrer"
                                            className="flex items-center justify-center w-full gap-2 rounded-xl bg-slate-800 py-3 text-xs font-semibold text-emerald-300 transition hover:bg-slate-700 hover:text-white border border-slate-700/50"
                                        >
                                            OpenStreetMap
                                        </a>
                                    )}
                            ...
                            <div className="relative z-10">
                                <div className="mb-6">
                                    ...
                                    <div className="mt-3 flex flex-wrap items-center gap-3">
                                        <span className={`px-3 py-1 rounded-xl text-xs font-semibold uppercase tracking-wider ${country.independent ? "bg-emerald-400/10 text-emerald-300 border border-emerald-400/20" : "bg-rose-400/10 text-rose-300 border border-rose-400/20"}`}>
                                            {country.independent ? "Independent" : "Dependent Territory"}
                                        </span>
                                        {country.unMember && <span className="px-3 py-1 rounded-xl text-xs font-semibold uppercase tracking-wider bg-cyan-400/10 text-cyan-300 border border-cyan-400/20">UN Member</span>}
                                        {country.landlocked && <span className="px-3 py-1 rounded-xl text-xs font-semibold uppercase tracking-wider bg-amber-400/10 text-amber-300 border border-amber-400/20">Landlocked</span>}
                                        <span className="px-3 py-1 rounded-xl text-xs font-bold text-slate-300 bg-slate-800 border border-slate-700">{country.cca2} / {country.cca3}</span>
                                    </div>
                                </div>
                                ...
                                <div className="grid gap-x-6 gap-y-4 text-sm text-slate-300 sm:grid-cols-2 sm:text-base">
                                    ...
                                    <p><span className="font-semibold text-slate-100">Status :</span> {country.status || "N/A"}</p>
                                    <p><span className="font-semibold text-slate-100">Coordinates :</span> {coords}</p>
                                    <p><span className="font-semibold text-slate-100">FIFA Code :</span> {country.fifa || "N/A"}</p>
                                    <p><span className="font-semibold text-slate-100">Gini Index :</span> {giniText}</p>
                                ...
                                <NavLink to="/country">
                                    <motion.button
                                        whileHover={{ x: -4 }}
                                        className="mt-8 inline-flex items-center gap-2 rounded-xl border border-cyan-300/40 bg-cyan-400/10 px-5 py-2.5 text-sm font-medium text-cyan-200 transition hover:bg-cyan-400/20"
                                    >
